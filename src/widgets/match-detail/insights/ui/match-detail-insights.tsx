...
export const MatchDetailInsights = ({ insights }: MatchDetailInsightsProps) => {
  ...
  return (
    <section className='bg-slate-900/60 border border-white/10 rounded-3xl backdrop-blur-2xl shadow-2xl overflow-hidden'>
      <header className='px-4 py-4 sm:px-8 sm:py-6 border-b border-white/10 flex flex-col md:flex-row md:items-center md:justify-between gap-3'>
        <div>
          <p className='text-xs uppercase tracking-[0.3em] text-slate-300'>Match Insights</p>
          <h2 className='text-xl sm:text-2xl font-bold text-white'>데이터로 보는 포인트</h2>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/10'>
        {insights.map((insight) => (
          <article key={insight.label} className='p-4 sm:p-6 md:p-8 space-y-4'>
            <div className='flex items-start justify-between gap-4'>
              <div className='space-y-3'>
                <p className='text-sm uppercase tracking-[0.25em] text-slate-300'>{insight.label}</p>
...
const ValueDisplay = ({ value }: { value: string }) => {
  ...
  if (!value.includes(separator)) {
    return <span className='text-2xl sm:text-3xl font-black text-white leading-tight'>{value}</span>;
  }

  const [home, away] = value.split(separator);

  return (
    <div className='inline-flex flex-col text-2xl sm:text-3xl font-black text-white leading-tight space-y-1'>
      <span className='whitespace-nowrap'>{home}</span>
      <span className='self-start rounded-full bg-slate-500/20 px-3 py-0.5 text-xs font-semibold uppercase tracking-[0.35em] text-slate-200'>vs</span>
